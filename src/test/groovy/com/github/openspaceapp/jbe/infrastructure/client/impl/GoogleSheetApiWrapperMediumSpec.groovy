...
class GoogleSheetApiWrapperMediumSpec extends Specification {
    ...
    def "instantiate object without exception"() {
        when:
            new GoogleSheetApiWrapper(APIKEY)
        then:
            notThrown Throwable
    }
    ...
    def "get values from an existing spreadsheet"() {
        given:
            def sheetApiWrapper = new GoogleSheetApiWrapper(APIKEY)
        when:
    ...
    def "expect exception when using invalid apikey"() {
        given:
            def sheetApiWrapper = new GoogleSheetApiWrapper("NotQuiteCorrectApiKey.Hopefully")
        when:
