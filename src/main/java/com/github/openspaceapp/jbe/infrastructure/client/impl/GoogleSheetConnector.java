...
public class GoogleSheetConnector implements SheetImporter {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(GoogleSheetConnector.class);
    ...
    public Optional<SheetImport> get(String spreadsheetId) {
        ...
        return getValuesForRange(spreadsheetId, "prog").map(this::createSheetImportFromRow);
    }
    ...
    private List<SheetRow> convertRowsExceptFirst(List<List<Object>> rows) {
        return rows.stream().skip(1).map(SheetRow::new).collect(Collectors.toList());
    }

    private List<String> extractHeaders(List<List<Object>> rows) {
        return rows.stream().findFirst().map(this::toStringList).orElse(Collections.emptyList());
    }

    private Optional<List<List<Object>>> getValuesForRange(String spreadsheetId, String range) {
        try {
            return Optional.of(sheets.getValues(spreadsheetId, range));
        } catch (IOException e) {
    ...
    private List<String> toStringList(List<Object> objects) {
        return objects.stream().map(x -> (String) x).collect(Collectors.toList());
    }
}
